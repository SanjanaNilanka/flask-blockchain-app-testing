...
def check_blocks_integrity():
    ...
    for i in range(2, int(get_next_block())):
        ...
        try:
            with open(f"{BLOCKCHAIN_DIR}{cur_index}.json", "r") as file:
        ...
        try:
            with open(BLOCKCHAIN_DIR + prev_index + ".json", "rb") as file:
                prev_hash = hashlib.sha256(file.read()).hexdigest()
        except Exception as e:
            print(e)
...
def check_block(index):
    ...
    tmp = {"block": "", "result": "", "proof": ""}
    try:
        with open(BLOCKCHAIN_DIR + cur_index + ".json", "r") as file:
            file_dict = json.load(file)
            cur_hash = file_dict["prev_hash"]
            cur_proof = file_dict["proof"]
    except Exception as e:
